	...
	public void function run(){
		describe( "useDBForMapping", () => {
			beforeEach( () => {
				...
				variables.ormSettings = { dbcreate : "update", useDbForMapping : true };
			} );

			it(
				title = "Can read table column data from the database",
				body  = () => {
					...
					var theTest = () => {
						queryExecute(
							"ALTER TABLE KitchenSink ADD #randomPropertyName# VARCHAR(#randomVarcharLen#) DEFAULT '#randomDefault#' NULL",
							{},
							{ "datasource" : "mysql" }
						);
						ormReload();
