    ...
    public void function run(){
        describe( "Global event listener (EventHandler.cfc)", () => {
            beforeEach( () => {
                ...
                variables.testAuto = entityNew( 'Auto',{
                    id : createUUID(),
                    make : "Lexus"
                } );
            ...
            it( "emits pre/post insert events", () => {
                entitySave( entityNew( 'Auto',{
                    id : createUUID(),
                    make : "BMW"
                } ) );
                ormFlush();
                // debug( application.ormEventLog );
                expect( application.ormEventLog.map( ( event ) => event.eventName ) )
                    .toBe( ["onFlush", "preInsert", "postInsert" ]);
            });
            ...
            it( "emits pre/post update events", () => {
                ...
                // debug( application.ormEventLog );
                expect( application.ormEventLog.map( ( event ) => event.eventName ) )
                    .toBe( ["onFlush", "preUpdate", "postUpdate" ]);
            });
            ...
            it( "emits pre/post delete events", () => {
                ...
                ormFlush();
                expect( application.ormEventLog.map( ( event ) => event.eventName ) )
                    .toBe( ["onDelete", "onFlush", "preDelete", "postDelete" ]);
            });
