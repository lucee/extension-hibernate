component {
		...
		switch ( arguments.datasource ) {
			case "mssql":
				...
				if ( structCount( msSql ) gt 0 ) {
					if ( arguments.onlyConfig ) return msSql;
					return {
			...
			case "mysql":
				...
				if ( structCount( mySql ) gt 0 ) {
					if ( arguments.onlyConfig ) return mySql;
					return {
			...
			case "postgres":
				...
				if ( structCount( pgsql ) gt 0 ) {
					if ( arguments.onlyConfig ) return pgsql;
					return {
			...
			case "h2":
				if ( arguments.verify ) {
					...
					if ( !directoryExists( tempDb ) ) directoryCreate( tempDb );
			...
			case "oracle":
				...
				if ( structCount( oracle ) gt 0 ) {
					if ( arguments.onlyConfig ) return oracle;
					return {
	...
	public function getSystemPropOrEnvVars(
		string props        = "",
		string prefix       = "",
		...
		loop list="environment,properties" item="src" {
			...
			for ( k in keys ) {
				...
				if ( !isNull( props[ k ] ) && len( trim( props[ k ] ) ) neq 0 ) {
					kk = k;
					if ( arguments.stripPrefix ) kk = mid( k, len( arguments.prefix ) + 1 ); // i.e. return DATABASE for MSSQL_DATABASE
					st[ kk ] = props[ k ];
				}
			}
			if ( structCount( st ) eq n ) break;
			else st = {};
		}
