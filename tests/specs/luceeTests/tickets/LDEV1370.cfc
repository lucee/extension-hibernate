...
component extends="org.lucee.cfml.test.LuceeTestCase" labels="orm" {
	...
	public void function testWithDefaultSettings(){
		...
		local.uri    = server.helpers.getTestPath( "tickets/LDEV1370/index.cfm" );
		local.result = _InternalRequest( uri );
		assertEquals( 200, result.status );
		assertEquals(
			"inital:1:1;explicit:1:1:explicit change;transaction:1:1:transaction change;",
			trim( result.fileContent )
		);

		// after end (second request)
		local.uri    = server.helpers.getTestPath( "tickets/LDEV1370/result.cfm" );
		local.result = _InternalRequest( uri );
		assertEquals( 200, result.status );
