...
component extends="org.lucee.cfml.test.LuceeTestCase" labels="orm"	{
	...
	public void function testCompositeId(){
		...
		local.uri=server.helpers.getTestPath("tickets/Jira2644/one.cfm");
		local.result=_InternalRequest(uri);
		assertEquals(200,result.status);
		assertEquals('',trim(result.filecontent));
		
		// now get the result
		local.uri=server.helpers.getTestPath("tickets/Jira2644/two.cfm");
		local.result=_InternalRequest(uri);
	...
	public void function testCompositeId2(){
		// first call only initialize the data
		local.uri=server.helpers.getTestPath("tickets/Jira2644/bestseller.cfm");
		local.result=_InternalRequest(uri);
		assertEquals(200,result.status);
	}
