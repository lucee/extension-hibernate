component extends="org.lucee.cfml.test.LuceeTestCase" labels="orm,cache,ehCache" {
	...
	function run( testResults, testBox ){
		describe(
			title = "Test suite for LDEV-1741",
			body  = function(){
				it(
					title = "checking ORM secondary ehcache with this.ormsettings.cacheconfig = ""ehcache.xml"" ",
				...
				it(
					title = "checking ORM secondary ehcache without cacheconfig",
					body  = function( currentSpec ){
						var uri     = server.helpers.getTestPath( "tickets/LDEV1741" );
						var result1 = _InternalRequest(
