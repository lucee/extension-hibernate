...
// notnull — table name casing varies by DB, try both
try {
	cfdbinfo( type="Columns", name="cols", table="KitchenSink" );
} catch ( any e ) {
	cfdbinfo( type="Columns", name="cols", table="KITCHENSINK" );
}
notnullCol = cols.filter( function( row ) { return uCase( row.COLUMN_NAME ) == "NOTNULLABLE"; } );
if ( notnullCol.NULLABLE[ 1 ] != 0 ) throw( message="notnullable column should not be nullable" );
nullableCol = cols.filter( function( row ) { return uCase( row.COLUMN_NAME ) == "NULLABLE"; } );
