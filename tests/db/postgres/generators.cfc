component extends="org.lucee.cfml.test.LuceeTestCase" labels="orm" {
	...
	function run( testResults, testBox ) {
		...
		describe( title="ORM ID generators [postgres]", skip=notHasPostgres(), body=function() {
			...
			it( "generator=sequence uses a named DB sequence", function() {
				var result = _InternalRequest( template: "#uri()#/sequence.cfm" );
				expect( trim( result.filecontent ) ).toBe( "ok" );
			});
