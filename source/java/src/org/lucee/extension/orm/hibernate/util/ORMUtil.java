...
public class ORMUtil {
    ...
    // /**
    //  * Override's Lucee's {@link lucee.runtime.PageContext#getORMSession(boolean)} method to avoid stupidly creating a new session if one already exists.
    //  * 
	...
	public static Struct convertToSimpleMap(String paramsStr) {
		...
		for (int i = 0; i < arr$.length; i++) {
			...
			params.setEL(
                CommonUtil.createKey(deleteQuotes(pair.substring(0, index).trim()).trim())
