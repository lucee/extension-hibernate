 ...
 * 
 * For example, a <code>postInsert()</code> method in a <code>User</code> entity will be invoked prior to the entity's insertion into the database. This same method will also be invoked on the EventHandler component, if configured.
 */
public class EventListenerIntegrator implements Integrator, 
	...
	PreUpdateEventListener, PostUpdateEventListener, PreLoadEventListener, PostLoadEventListener, 
	...
	ClearEventListener, DirtyCheckEventListener, EvictEventListener
{
	 ...
	 * <p>
	 * Note this listener will ONLY fire for events related to itself.
	 * i.e. Car.preInsert will be run for Car entity preInsert events - not for User entity preInsert events.
	 * 
	 * @param cfc Instantiated Lucee Component object.
	 */
	...
	public boolean onPreInsert(PreInsertEvent event) {
		Struct state = entityStateToStruct(
		...
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.PRE_INSERT, event, state ); }
	...
	public void onPostInsert(PostInsertEvent event) {
		...
		// fire on entity
		Component listener = getEventListener(event.getEntity());
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.POST_INSERT, event, null ); }
	}
	...
	public boolean onPreDelete(PreDeleteEvent event) {
		...
		Component listener = getEventListener(event.getEntity());
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.PRE_DELETE, event, null ); }
		return false;
	}
	...
	public void onPostDelete(PostDeleteEvent event) {
		...
		Component listener = getEventListener(event.getEntity());
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.POST_DELETE, event, null ); }
	}

	// PreUpdateEventListener
	@Override
	public boolean onPreUpdate(PreUpdateEvent event) {
		Struct oldState = entityStateToStruct(
				event.getPersister().getPropertyNames(),
				event.getOldState()
		);
		...
		Component listener = getEventListener(event.getEntity());
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.PRE_UPDATE, event, oldState ); }
		return false;
	}
	...
	public void onPostUpdate(PostUpdateEvent event) {
		...
		Component listener = getEventListener(event.getEntity());
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.POST_UPDATE, event, null ); }
	}
	...
	public void onPreLoad(PreLoadEvent event) {
		...
		Component listener = getEventListener(event.getEntity());
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.PRE_LOAD, event, null ); }
	}
	...
	public void onPostLoad(PostLoadEvent event) {
		...
		Component listener = getEventListener(event.getEntity());
		if ( listener != null ) { fireEventOnEntityListener( listener, CommonUtil.POST_LOAD, event, null ); }
	}
	 ...
	 * 
	 * @param comp Lucee Component to look on, for example events.EventHandler.
	 * @param methodName Method name to look for, for example "preInsert"
	...
	private static void _fireOnComponent( Component cfc, Key name, Object... args ){
		if ( !componentHasMethod( cfc, name ) ){ return; }
	 ...
	 * @param values Array of property values, either retrieved from <code>event.getPersister().getPropertyValues()</code>, <code>event.getState()</code> or <code>event.getOldState()</code>
	 * @return A struct
	 */
